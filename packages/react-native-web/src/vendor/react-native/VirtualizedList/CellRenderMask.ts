...
export class CellRenderMask {
  ...
  addCells(cells: { first: number, last: number }): void {
    ...
    if (firstIntersect && firstIntersect.first < newMainRegion.first) {
    ...
    if (lastIntersect && lastIntersect.last > newMainRegion.last) {
    ...
    if (firstIntersectIdx && lastIntersectIdx) {
  ...
  _findRegion(cellIdx: number): [CellRegion, number] | void {
    let firstIdx = 0;
