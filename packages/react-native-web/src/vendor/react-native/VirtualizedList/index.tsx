...
import type { LayoutEvent } from '../../../types/index';
import type { ScrollEvent } from '../Types/CoreEventTypes';
import type { ViewToken, ViewabilityConfig } from '../ViewabilityHelper/index';
import type {
  FrameMetricProps,
  Item,
  Props,
...
import RefreshControl from '../../../exports/RefreshControl/index';
import ScrollView, {
  type ScrollViewRef
} from '../../../exports/ScrollView/index';
import View, { type ViewProps } from '../../../exports/View/index';
import StyleSheet from '../../../exports/StyleSheet/index';

import Batchinator from '../Batchinator/index';
import clamp from '../Utilities/clamp';
import infoLog from '../infoLog/index';
import { CellRenderMask } from './CellRenderMask';
import ChildListCollection from './ChildListCollection';
import FillRateHelper from '../FillRateHelper/index';
import StateSafePureComponent from './StateSafePureComponent';
import ViewabilityHelper from '../ViewabilityHelper/index';
import CellRenderer, { SeparatorProps } from './VirtualizedListCellRenderer';
import {
...
} from '../VirtualizeUtils/index';
import invariant from '../../../modules/invariant/index';
...
function horizontalOrDefault(horizontal: boolean | undefined) {
  return horizontal ?? false;
}

// initialNumToRenderOrDefault(this.props.initialNumToRender)
function initialNumToRenderOrDefault(initialNumToRender: number | undefined) {
  return initialNumToRender ?? 10;
}

// maxToRenderPerBatchOrDefault(this.props.maxToRenderPerBatch)
function maxToRenderPerBatchOrDefault(maxToRenderPerBatch: number | undefined) {
  return maxToRenderPerBatch ?? 10;
}

// onStartReachedThresholdOrDefault(this.props.onStartReachedThreshold)
function onStartReachedThresholdOrDefault(
...
function scrollEventThrottleOrDefault(scrollEventThrottle: number | undefined) {
  return scrollEventThrottle ?? 50;
}

// windowSizeOrDefault(this.props.windowSize)
function windowSizeOrDefault(windowSize: number | undefined) {
...
class VirtualizedList<
  Data extends readonly any[] = readonly any[]
> extends StateSafePureComponent<
  Props<Data>,
  State,
  VirtualizedListContextType<Data>
> {
   ...
   */
  getScrollResponder(): ScrollResponderType | undefined {
    if (this._scrollRef && this._scrollRef.getScrollResponder) {
      return this._scrollRef.getScrollResponder();
    }
  }

  getScrollableNode(): ScrollViewRef | null | undefined {
  ...
  _getOutermostParentListRef = () => {
    if (this._isNestedWithSameOrientation()) {
      return this.context?.getOutermostParentListRef() || this;
    } else {
      return this;
    }
  };
  ...
  constructor(props: Props<Data>) {
    ...
    const initialRenderRegion =
      VirtualizedList._initialRenderRegion<Data>(props);

    this.state = {
      cellsAroundViewport: initialRenderRegion,
      renderMask: VirtualizedList._createRenderMask<Data>(
        props,
        initialRenderRegion
      )
    };
  ...
  static _createRenderMask<Data extends readonly any[]>(
    ...
    cellsAroundViewport: { first: number; last: number },
    additionalRegions?:
      | ReadonlyArray<{ first: number; last: number }>
      | undefined
  ): CellRenderMask {
  ...
  static _initialRenderRegion<Data extends readonly any[]>(
    props: Props<Data>
  ...
  invertedWheelEventHandler = (ev: WheelEvent) => {
    ...
    const delta = this.props.horizontal
      ? // @ts-expect-error old deprecated property
  ...
  componentWillUnmount() {
    if (this._isNestedWithSameOrientation() && this.context) {
  ...
  _onUpdateSeparators = (
    ...
    newProps: Partial<SeparatorProps<any>>
  ) => {
    keys.forEach((key) => {
  ...
  _cellRefs: { [key: string]: null | CellRenderer<any> } = {};
  ...
  _scrollRef: ScrollViewRef | null = null;
  ...
  _captureScrollRef = (ref: ScrollViewRef) => {
    this._scrollRef = ref;
  };
  ...
  measureLayoutRelativeToContainingList(): void {
    ...
    try {
      if (!this._scrollRef || !this.context) {
        return;
      }
  ...
  _renderDebugOverlay() {
    ...
    const framesInLayout: ReturnType<typeof this.__getFrameMetricsApprox>[] =
      [];
    const itemCount = this.props.getItemCount(this.props.data);
    for (let ii = 0; ii < itemCount; ii++) {
      const frame = this.__getFrameMetricsApprox(ii, this.props);
      if ('inLayout' in frame && frame.inLayout) {
...
export type { Props as VirtualizedListProps };
