...
const ModalFocusTrap = ({
  active,
  children
}: ModalFocusTrapProps): React.ReactNode => {
  const trapElementRef = React.useRef<ViewRef | null>(null);
  ...
  React.useEffect(() => {
    if (canUseDOM) {
      const trapFocus = () => {
        ...
        focusRef.current.lastFocusedElement = document.activeElement as HTMLElement | null;
      };
