...
const activeModalListeners: Record<string, (isActive: boolean) => void> = {};
...
function addActiveModal(modalId: string, listener: (isActive: boolean) => void) {
...
const Modal = React.forwardRef((props: ModalProps, forwardedRef: React.Ref<ViewRef>) => {
  const {
    animationType,
    children,
    onDismiss,
  ...
  const modalId = React.useMemo(() => String(uniqueModalIdentifier++), []);
