...
const Switch = React.forwardRef(
  (props: SwitchProps, forwardedRef: React.Ref<ViewRef>) => {
    const {
    ...
    const thumbRef = React.useRef<ViewRef>(null);

    function handleChange(event: React.ChangeEvent<HTMLInputElement>) {
      if (onValueChange != null) {
        onValueChange((event.nativeEvent.target as HTMLInputElement).checked);
      }
    }

    function handleFocusState(event: React.FocusEvent<HTMLInputElement>) {
      const isFocused = event.nativeEvent.type === 'focus';
      const boxShadow = isFocused
        ? thumbFocusedBoxShadow
        : thumbDefaultBoxShadow;
    ...
    return (
      <View {...other} style={rootStyle}>
        ...
        <View
          ref={thumbRef}
          // @ts-expect-error DOM vs RN style clash
          style={thumbStyle}
        />
        {nativeControl}
      </View>
