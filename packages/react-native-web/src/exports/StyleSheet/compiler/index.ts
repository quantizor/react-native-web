...
type RulesData = Array<Rules | number>;
type CompiledStyle = {
...
const cache = new Map<string, [identifier: string, rules: RulesData]>();
const emptyObject = {};
...
export function atomic(style: Style): CompilerOutput {
  const compiledStyle = { $$css: true } as CompiledStyle;
  const compiledRules: RulesData[] = [];
