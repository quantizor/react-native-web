...
function getEventPath(domEvent: Event): Array<any> {
...
function composedPathFallback(target?: Node | null): Array<Node> {
  const path: Node[] = [];
 ...
 */
export function getResponderPaths(domEvent: Event): {
  idPath: number[];
  nodePath: Node[];
} {
  const idPath: number[] = [];
