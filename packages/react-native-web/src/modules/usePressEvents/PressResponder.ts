...
export type PressResponderConfig = {
  ...
  delayPressStart?: number | undefined,
  // Duration to wait after letting up before calling `onPressEnd`.
  delayPressEnd?: number | undefined,
  // Called when a long press gesture has been triggered.
  onLongPress?: ((event: ResponderEvent) => void) | undefined,
  // Called when a press gestute has been triggered.
  onPress?: ((event: ClickEvent) => void) | undefined,
  // Called when the press is activated to provide visual feedback.
  onPressChange?: ((event: ResponderEvent) => void) | undefined,
  // Called when the press is activated to provide visual feedback.
  onPressStart?: ((event: ResponderEvent) => void) | undefined,
  // Called when the press location moves. (This should rarely be used.)
  onPressMove?: ((event: ResponderEvent) => void) | undefined,
  // Called when the press is deactivated to undo visual feedback.
  onPressEnd?: ((event: ResponderEvent) => void) | undefined
};
...
export default class PressResponder {
  ...
  _longPressDelayTimeout: number | null;
  _longPressDispatched: boolean;
  _pressDelayTimeout: number | null;
  _pressOutDelayTimeout: number | null;
  ...
  _createEventHandlers(): EventHandlers {
    const start = (event: ResponderEvent, shouldDelay?: boolean): void => {
      ...
      const delayPressStart = normalizeDelay(
        this._config.delayPressStart ?? null,
        0,
      ...
      if (shouldDelay !== false && delayPressStart > 0) {
        ...
        }, delayPressStart) as unknown as number;
      } else {
        this._receiveSignal(DELAY, event);
      }

      const delayLongPress = normalizeDelay(
        this._config.delayLongPress ?? null,
        10,
        DEFAULT_LONG_PRESS_DELAY_MS
      );
      this._longPressDelayTimeout = setTimeout(() => {
        this._handleLongPress(event);
      }, delayLongPress + delayPressStart) as unknown as number;
    };
  ...
  _deactivate(event: ResponderEvent): void {
    ...
    const delayPressEnd = normalizeDelay(this._config.delayPressEnd ?? null);
    if (delayPressEnd > 0) {
      this._pressOutDelayTimeout = setTimeout(() => {
        end();
      }, delayPressEnd) as unknown as number;
    } else {
      end();
    }
  }
